...
public sealed class FfmpegProcessRunner : IFfmpegProcessRunner
{
        ...
        CancellationToken cancellationToken = default)
    {
        var command = new ProcessCommand(
            ...
            BuildFfprobeArguments(inputFileUrl));
    ...
    private string BuildFfmpegHlsArguments(string inputFileUrl, string outputDirectory)
    {
        ...
        return $"-y -stats -loglevel error {hwaccel}-i \"{inputFileUrl}\" " +
               ...
               $"-hls_segment_filename \"{Path.Combine(outputDirectory, VideoAsset.SEGMENT_FILE_PATTERN)}\" " +
               $"-master_pl_name {VideoAsset.MASTER_PLAYLIST_NAME} " +
               $"\"{Path.Combine(outputDirectory, VideoAsset.STREAM_PLAYLIST_PATTERN)}\"";
    }
